...
use zero2prod::issue_delivery_worker::{try_execute_task, ExecutationOutcome};
...
impl TestApp {
    ...
    pub async fn dispatch_all_pending_emails(&self) {
        loop {
            if let ExecutationOutcome::EmptyQueue = 
...
pub async fn spawn_app() -> TestApp {
    ...
    let appliaction_port = application.port(); 
    ...
    let test_app = TestApp {
        address: format!("http://localhost:{}", appliaction_port),
        port: appliaction_port,
