...
pub struct AppliactionBaseUrl(pub String);
...
async fn run(
    ...
    let base_url = Data::new(AppliactionBaseUrl(base_url));
    let secret_key = Key::from(hmac_secret.expose_secret().clone().as_bytes());
